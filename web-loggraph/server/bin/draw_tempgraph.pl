...
#    print "<p>$timeMin, $timeMax, $tempMin, $tempMax</p>";
    ...
    my $colorWhite_3 = $image_3->colorAllocate( 255, 255, 255 );
...
#    $timeMax = timelocal( 0, 0, 0, 15, 4 - 1, 2018 - 1900 );  # 2018/1/2 0:00:00
    ...
    foreach my $arr (@dummy) {
        ...
        $x1 = $arr->[0];
        $y1 = $arr->[4];
        if ( $pressureMax > $y1 && $y1 > $pressureMin ) {
            convParam( \$y1, \$x1, $timeMin, $timeMax, $pressureMin, $pressureMax );
            $image_3->arc( $x1, $y1, 2, 2, 0, 360, $colorPurple );
        }
    }
